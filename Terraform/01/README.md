...
2. Скачайте на свой ПК этот git-репозиторий. Исходный код для выполнения задания расположен в директории **Terraform/01/src**.
...
`Вторая ошибка говорит о не корректном имени ресурса. По умолчанию, имя может начинаться только с буквы или подчеркивания. В нашем случае **1nginx** - не верно. Так же в теле данного блока была ошибка в значении аргумента: **name  = "example_${random_password.random_string_FAKE.resulT}"** - не верно, должно быть так: **name  = "example_${random_password.random_string.result}"**` - но выдавать ранее сгенерированный пароль в имени контейнера это не очень хорошая затея, исправил.
